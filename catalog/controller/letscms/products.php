...
class ControllerLetscmsProducts extends RestController {
  ...
  public function index()
  {
    ...
    if ( $_SERVER['REQUEST_METHOD'] === 'POST' ){
        ...
        if (isset($requestjson['filter'])) {
          $filter = $requestjson['filter'];
        } else {
          $filter = '';
        }

        if (isset($requestjson['sort'])) {
          $sort = $requestjson['sort'];
        } else {
          $sort = 'p.sort_order';
        }

        if (isset($requestjson['order'])) {
        ...
        if (isset($requestjson['limit'])) {
          $limit = (int)$requestjson['limit'];
        } else {
          $limit = $this->config->get($this->config->get('config_theme') . '_product_limit');
        }

        if (isset($requestjson['category_id'])) {
          $category_id = $requestjson['category_id'];
        } else {
          $category_id = 0;
        }
        ...
        if ($category_info) {
          ...
          $datalanguage['heading_title'] = $category_info['name'];
    			$datalanguage['text_refine'] = $this->language->get('text_refine');
    			$datalanguage['text_empty'] = $this->language->get('text_empty');
    			$datalanguage['text_quantity'] = $this->language->get('text_quantity');
    			$datalanguage['text_manufacturer'] = $this->language->get('text_manufacturer');
    			$datalanguage['text_model'] = $this->language->get('text_model');
    			$datalanguage['text_price'] = $this->language->get('text_price');
    			$datalanguage['text_tax'] = $this->language->get('text_tax');
    			$datalanguage['text_points'] = $this->language->get('text_points');
    			$datalanguage['text_compare'] = sprintf($this->language->get('text_compare'), (isset($this->session->data['compare']) ? count($this->session->data['compare']) : 0));
    			$datalanguage['text_sort'] = $this->language->get('text_sort');
    			$datalanguage['text_limit'] = $this->language->get('text_limit');
    			$datalanguage['button_cart'] = $this->language->get('button_cart');
    			$datalanguage['button_wishlist'] = $this->language->get('button_wishlist');
    			$datalanguage['button_compare'] = $this->language->get('button_compare');
    			$datalanguage['button_continue'] = $this->language->get('button_continue');
    			$datalanguage['button_list'] = $this->language->get('button_list');
    			$datalanguage['button_grid'] = $this->language->get('button_grid');
          $data['data'] = array();
          ...
          $product_data=array();
            ...
            $product_info = $this->model_catalog_product->getProduct($product_result['product_id']);
            if ($product_info) {
              $product_data['product_id'] = (int)$product_result['product_id'];
              $product_data['name'] = $product_result['name'];
